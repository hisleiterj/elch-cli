...
namespace Elchwinkel.CLI
{
    public class Table
    {
        ...
        public Table(params string[] columnHeaders)
            : this(new TableOptions { Columns = new List<string>(columnHeaders) })
        {
        }
        ...
        public Table AddColumn(IEnumerable<string> names)
        {
        ...
        public Table AddRow(params object[] values)
        {
            ...
            if (!Columns.Any())
                throw new Exception("Please set the columnHeaders first");

            if (Columns.Count != values.Length)
                throw new Exception(
                    $"The number columnHeaders in the row ({Columns.Count}) does not match the values ({values.Length}");

            Rows.Add(values);
            return this;
        }

        public Table Configure(Action<TableOptions> action)
        {
            action(Options);
            return this;
        }
        ...
        private string ToMarkDownString(char delimiter)
        {
            ...
            // create the string format with padding
            var format = _Format(columnLengths, delimiter);
        ...
        public string ToStringAlternative()
        {
            ...
            // create the string format with padding
            var format = _Format(columnLengths);
        ...
        private string _Format(List<int> columnLengths, char delimiter = '|')
        {
            // set right alinment if is a number
            var columnAlignment = Enumerable.Range(0, Columns.Count)
                .Select(GetNumberAlignment)
        ...
        public void Write(Format format = Format.Default)
        {
            switch (format)
            {
                case Format.Default:
        ...
        private static IEnumerable<string> _GetColumns<T>()
        {
